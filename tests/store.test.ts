...
describe('whimsicality-db', () => {
  ...
  describe('entries (unified store)', () => {
    it('saves and reads small text', async () => {
      ...
      const result = parsed<{ content: string; title: string; is_compressed: boolean }>(await mcp.call('tools/call', { name: 'db_entry_read', arguments: { id: 'doc1' } }))
      expect(result.content).toBe('Hello world')
      expect(result.title).toBe('Greeting')
      expect(result.is_compressed).toBe(false)
    })

    it('auto-compresses large content', async () => {
      const mcp = await readyServer()
      const content = 'The quick brown fox jumps over the lazy dog. '.repeat(2000)
      const storeResult = parsed<{ id: string; original_size: number; compressed_size: number; ratio: number }>(await mcp.call('tools/call', { name: 'db_entry_save', arguments: { id: 'big', text: content, title: 'Repeated fox' } }))
      expect(storeResult.original_size).toBeGreaterThan(storeResult.compressed_size)
      expect(storeResult.ratio).toBeLessThan(0.5)
      const readResult = parsed<{ content: string; is_compressed: boolean; total_length: number; has_more: boolean }>(await mcp.call('tools/call', { name: 'db_entry_read', arguments: { id: 'big', length: content.length } }))
      expect(readResult.content).toBe(content)
      expect(readResult.is_compressed).toBe(true)
    ...
    it('pages large content', async () => {
      ...
      const page1 = parsed<{ content: string; has_more: boolean }>(await mcp.call('tools/call', { name: 'db_entry_read', arguments: { id: 'paged', offset: 0, length: 100 } }))
      expect(page1.content).toBe(content.slice(0, 100))
      expect(page1.has_more).toBe(true)
      const page2 = parsed<{ content: string; has_more: boolean }>(await mcp.call('tools/call', { name: 'db_entry_read', arguments: { id: 'paged', offset: 9900, length: 200 } }))
    ...
    it('stores and filters by tags', async () => {
      ...
      const byTag = parsed<{ results: { entry_id: string }[] }>(await mcp.call('tools/call', { name: 'db_entry_by_tags', arguments: { tags: ['security'] } }))
      expect(byTag.results.length).toBe(2)
      const ids = byTag.results.map((r) => r.entry_id)
      expect(ids).toContain('a')
      expect(ids).toContain('c')
      const listFiltered = parsed<{ results: { entry_id: string }[] }>(await mcp.call('tools/call', { name: 'db_entry_list', arguments: { tag: 'auth' } }))
      expect(listFiltered.results.length).toBe(1)
      expect(listFiltered.results[0]?.entry_id).toBe('a')
    })
    ...
    it('updates entries and replaces tags', async () => {
      ...
      const oldResult = parsed<{ results: { entry_id: string }[] }>(await mcp.call('tools/call', { name: 'db_entry_list', arguments: { tag: 'old' } }))
      expect(oldResult.results.length).toBe(0)
      const newResult = parsed<{ results: { entry_id: string }[] }>(await mcp.call('tools/call', { name: 'db_entry_list', arguments: { tag: 'new' } }))
      expect(newResult.results[0]?.entry_id).toBe('e')
    })
  ...
  describe('sessions and events', () => {
    it('creates sessions and logs events', async () => {
      ...
      await mcp.call('tools/call', { name: 'db_event_log', arguments: { session_id: 'task-1', event_type: 'decision', content: 'Chose SQLite' } })
      await mcp.call('tools/call', { name: 'db_event_log', arguments: { session_id: 'task-1', event_type: 'milestone', content: 'Schema done' } })
      const events = parsed<{ results: { event_type: string }[] }>(await mcp.call('tools/call', { name: 'db_event_list', arguments: { session_id: 'task-1' } }))
  ...
  describe('stats and cross-process', () => {
    it('returns database stats', async () => {
      ...
      const stats = parsed<{ memory_count: number; entry_count: number; todo_count: number; session_count: number; event_count: number; db_size_bytes: number }>(await mcp.call('tools/call', { name: 'db_stats', arguments: {} }))
  ...
  describe('import from whimsicality-mcp', () => {
    it('imports memory and docs from JSON store', async () => {
      ...
      try {
        ...
        const result = parsed<{ memory: number; entries: number; errors: string[] }>(await mcp.call('tools/call', { name: 'db_import', arguments: { source_dir: sourceDir } }))
