...
const rawProp = { type: 'boolean', default: false, description: 'If true, pass query directly to FTS5 (supports AND, OR, NOT, NEAR, *). Default: false (safe term extraction).' }
const tagFilterProp = { type: 'string', maxLength: MAX_TAG_CHARS, description: 'Filter by exact tag match' }
...
const TOOLS: readonly ToolDef[] = [
  ...
  { name: 'db_memory_search', description: 'FTS5 full-text search across all memory values. Returns ranked matches with BM25 scores (higher = better). Safe for natural-language queries by default.', inputSchema: schema({ query: str('Search query', 10_000), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
  ...
  { name: 'db_doc_search', description: 'FTS5 search over documents. Returns match-centered excerpts with BM25 scores (higher = better).', inputSchema: schema({ query: str('Search query', 10_000), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
  ...
  { name: 'db_cache_index', description: 'Get a compact summary table of cached content. Token cost printed at bottom. Use to discover what is available before reading.', inputSchema: schema({ topic: optStr('Optional topic filter (substring match)', 80), tag: tagFilterProp, limit: limitProp }), annotations: RO },
  { name: 'db_cache_search', description: 'FTS5 search over cache index (topic, summary, tags). Returns ranked summaries with BM25 scores (higher = better).', inputSchema: schema({ query: str('Search query', 10_000), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
  ...
  { name: 'db_todo_list', description: 'List todos, optionally filtered by status, tag, or session. Ordered by priority (desc) then created_at (asc).', inputSchema: schema({ status: optStr('Filter by status: pending, in_progress, completed', 20), tag: tagFilterProp, session_id: optStr('Filter by session', MAX_IDENTIFIER_CHARS), limit: limitProp }), annotations: RO },
  { name: 'db_todo_update', description: 'Update a todo item. Any field can be updated; omitted fields are preserved. Use status=completed to mark done. Pass empty string to clear title or description.', inputSchema: schema({ id: { type: 'integer', description: 'Todo ID' }, status: optStr('New status: pending, in_progress, completed', 20), title: optStr('New title (pass empty string to clear)', 10_000), description: optStr('New description (pass empty string to clear)', MAX_TEXT_CHARS), priority: priorityProp }, ['id']), annotations: WRITE },
  ...
  { name: 'db_todo_search', description: 'FTS5 search over todo titles and descriptions. Returns ranked matches with BM25 scores (higher = better).', inputSchema: schema({ query: str('Search query', 10_000), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
  ...
  { name: 'db_context_search', description: 'FTS5 search over context entries (title, content, tags). Returns ranked matches with BM25 scores (higher = better).', inputSchema: schema({ query: str('Search query', 10_000), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
  ...
  { name: 'db_session_update', description: 'Update a session. Use status=completed/abandoned/paused to change lifecycle. Pass empty string to clear name or description.', inputSchema: schema({ id: idProp, status: optStr('New status: active, paused, completed, abandoned', 20), name: optStr('New name (pass empty string to clear)', 10_000), description: optStr('New description (pass empty string to clear)', MAX_TEXT_CHARS) }, ['id']), annotations: WRITE },
  ...
  { name: 'db_event_search', description: 'FTS5 search over event content. Returns ranked matches with match-centered excerpts. BM25 scores (higher = better).', inputSchema: schema({ query: str('Search query', 10_000), session_id: optStr('Filter to a specific session', MAX_IDENTIFIER_CHARS), topK: topKProp, raw: rawProp }, ['query']), annotations: RO },
...
export function dispatch(store: Store, name: string, args: Record<string, unknown>): unknown {
  ...
  const optText = (n: string): string => (args[n] === undefined || args[n] === null ? '' : String(args[n]))
  ...
  switch (name) {
    ...
    case 'db_memory_search': return { results: store.memorySearch(text('query', 10_000), topK(), raw()) }
    // Docs
    ...
    case 'db_doc_search': return { results: store.docSearch(text('query', 10_000), topK(), raw()) }
    ...
    case 'db_cache_index': return { table: store.cacheIndex(args.topic ? String(args.topic) : null, optNullable('tag'), limit()) }
    case 'db_cache_search': return { results: store.cacheSearch(text('query', 10_000), topK(), raw()) }
    ...
    // Todos
    case 'db_todo_add': return store.todoAdd(text('title', 10_000), optText('description'), intArg('priority') ?? 0, tags(), optNullable('session_id'))
    case 'db_todo_list': return { results: store.todoList(optNullable('status'), optNullable('tag'), optNullable('session_id'), limit()) }
    ...
    case 'db_todo_search': return { results: store.todoSearch(text('query', 10_000), topK(), raw()) }
    // Context
    ...
    case 'db_context_search': return { results: store.contextSearch(text('query', 10_000), topK(), raw()) }
    ...
    case 'db_session_list': return { results: store.sessionList(optNullable('status'), limit()) }
    case 'db_session_update': return store.sessionUpdate(id('id'), optNullable('status'), optNullable('name'), optNullable('description'))
    // Events
    case 'db_event_log': return store.eventLog(optNullable('session_id'), text('event_type', 256), text('content', MAX_TEXT_CHARS), optText('metadata') || '{}')
    case 'db_event_list': return { results: store.eventList(optNullable('session_id'), optNullable('event_type'), limit()) }
    case 'db_event_search': return { results: store.eventSearch(text('query', 10_000), optNullable('session_id'), topK(), raw()) }
    // Stats
