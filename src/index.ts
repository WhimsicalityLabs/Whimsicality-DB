...
const TOOLS: readonly ToolDef[] = [
  ...
  { name: 'db_entry_save', description: 'Store text. Auto-compresses if large (>64KB). Tags and source optional. Use for docs, references, or large content.', inputSchema: schema({ id: idP, text: s('Text content', MAX_CONTENT_CHARS), title: os('Title/summary', 200), tags: tagsP, source: os('Source (file, URL, etc.)', 256), compress: { type: 'boolean', description: 'Force compression' } }, ['id', 'text']), annotations: WRITE },
  { name: 'db_entry_read', description: 'Read entry by ID. Supports paging via offset+length.', inputSchema: schema({ id: idP, offset: offsetP, length: lengthP }, ['id']), annotations: RO },
  { name: 'db_entry_list', description: 'List entries. Optional tag filter.', inputSchema: schema({ tag: tagP, limit: limitP }), annotations: RO },
  { name: 'db_entry_by_tags', description: 'Get entries matching any of the given tags.', inputSchema: schema({ tags: { type: 'array', items: { type: 'string' }, description: 'Tags to match' }, limit: limitP }, ['tags']), annotations: RO },
  { name: 'db_entry_delete', description: 'Delete an entry.', inputSchema: schema({ id: idP }, ['id']), annotations: DELETE },

  { name: 'db_todo_add', description: 'Add a todo with priority, tags, session link.', inputSchema: schema({ title: s('Title', 10_000), description: os('Description', MAX_TEXT_CHARS), priority: priorityP, tags: tagsP, session_id: os('Session ID', MAX_IDENTIFIER_CHARS) }, ['title']), annotations: WRITE },
  { name: 'db_todo_list', description: 'List todos. Filter by status/tag/session.', inputSchema: schema({ status: os('pending|in_progress|completed', 20), tag: tagP, session_id: os('Session ID', MAX_IDENTIFIER_CHARS), limit: limitP }), annotations: RO },
  ...
  { name: 'db_session_create', description: 'Create or update a session for long-horizon tasks.', inputSchema: schema({ id: idP, name: s('Session name', 10_000), description: os('Description', MAX_TEXT_CHARS) }, ['id', 'name']), annotations: WRITE },
  { name: 'db_session_list', description: 'List sessions. Optional status filter. Pass id to get one session.', inputSchema: schema({ id: os('Session ID to get', MAX_IDENTIFIER_CHARS), status: os('active|paused|completed|abandoned', 20), limit: limitP }), annotations: RO },
  ...
  { name: 'db_event_log', description: 'Log an event in a session. FTS5-searchable.', inputSchema: schema({ session_id: os('Session ID', MAX_IDENTIFIER_CHARS), event_type: s('Type: decision|milestone|error|note|observation', 256), content: s('Event content', MAX_TEXT_CHARS), metadata: os('JSON metadata', MAX_TEXT_CHARS) }, ['event_type', 'content']), annotations: WRITE },
  { name: 'db_event_list', description: 'List events. Filter by session/type.', inputSchema: schema({ session_id: os('Session ID', MAX_IDENTIFIER_CHARS), event_type: os('Type filter', 256), limit: limitP }), annotations: RO },

  { name: 'db_search', description: 'Unified FTS5 search across collections. Returns ranked results with BM25 scores (higher=better).', inputSchema: schema({ query: s('Search query', 10_000), collections: collectionsP, top_k: topKP, raw: rawP }, ['query']), annotations: RO },
  { name: 'db_stats', description: 'Database statistics: counts and size.', inputSchema: schema({}), annotations: RO },
  { name: 'db_import', description: 'Import data from whimsicality-mcp storage directory.', inputSchema: schema({ source_dir: s('Path to whimsicality-mcp storage dir', 1024) }, ['source_dir']), annotations: WRITE },
]
...
export function dispatch(store: Store, name: string, args: Record<string, unknown>): unknown {
  ...
  const topK = (): number => { const v = args.top_k ?? 10; if (!Number.isInteger(v) || (v as number) < 1 || (v as number) > 100) throw new Error('top_k must be 1-100'); return v as number }
  ...
  switch (name) {
    ...
    case 'db_todo_add': return store.todoAdd(text('title', 10_000), os('description'), intArg('priority') ?? 0, tags(), optNullable('session_id'))
    ...
    case 'db_event_log': return store.eventLog(optNullable('session_id'), text('event_type', 256), text('content', MAX_TEXT_CHARS), os('metadata') || '{}')
    ...
    case 'db_import': return store.importMcp(String(args.source_dir ?? ''))
